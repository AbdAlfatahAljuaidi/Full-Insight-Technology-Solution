...
const MostQuestions = () => {
  ...
  return (
    <div className="px-6 w-full h-full my-16">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-extrabold text-[#0078B8]">Frequently Asked Questions</h1>
        <h2 className="text-xl text-gray-700 mt-2">Answers to your most common questions</h2>

        <div className="mt-6 space-y-4">
          {questions.map((question) => (
            <div key={question.id} className="border-b pb-4 transition-all">
              <button
                className="flex items-center justify-between w-full text-lg font-semibold text-[#0078B8] hover:text-hover focus:outline-none"
              ...
              {openAnswer === question.id && (
                <div className="mt-2 text-gray-800 transition-all">{question.answer}</div>
              )}
            </div>
          ))}
        </div>
  ...
  <h1 className="text-3xl font-semibold text-gray-900">Still have questions?</h1>
  <p className="text-lg text-gray-500 mt-2">Can’t find the answer you’re looking for? Please chat to our friendly team.</p>
  <Link to={'/Contact'}> <button className="bg-[#0078B8] text-white px-8 py-4 mt-4 rounded-full hover:bg-hover focus:outline-none transition-colors duration-300">
Get In Touch
  </button></Link>
</div>
